...
import {InitKplConceptECharts} from "@/Api/echarts/index/kpl_concept";
...
const axiosEcharts = ()=>{
  ...
  axios.post("http://127.0.0.1:8081/index/kpl_concept",query_dic).then(
      response=>{
        // alert(JSON.stringify(response.data));
        try{
          InitKplConceptECharts(response.data,route,router);
        }
        catch (err){
          alert("echarts处理异常")
        }
