...
import {InitDcIndexECharts} from "@/Api/echarts/index/dc_index";
...
const axiosEcharts = ()=>{
  ...
  axios.post("http://127.0.0.1:8081/index/dc_index",query_dic).then(
      response=>{
        alert(JSON.stringify(response.data));
        try{
          InitDcIndexECharts(response.data,route,router);
        }
        catch (err){
          alert("echarts处理异常")
        }
