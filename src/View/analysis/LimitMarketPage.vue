  ...
  <el-container>
    ...
    <el-table
      ...
      <el-table-column
          ...
          width="90">
        <template #default="{ row }">
          <el-link :href="`/#/market/daily?select_id=1&p\ara_id=20&level=analysis&id=${row.idx}&trade_date=${row.trade_date}`" style="color: gray" target="_blank">{{ row.name }}</el-link>
        </template>
      </el-table-column>
...
<!--          <a :href="market" style="color: gray">{{ row.industry_name_l3 }}</a>-->
      ...
      <el-table-column width="70">
        ...
        <template #default="{ row }">
          <a v-if="row.open_chg > 0" :href="market" style="color: red">{{ row.open_chg }}</a>
          <a v-if="row.open_chg == 0" :href="market" style="color: gray">{{ row.open_chg }}</a>
          <a v-if="row.open_chg < 0" :href="market" style="color: green">{{ row.open_chg }}</a>
        </template>
      </el-table-column>
      <el-table-column width="70">
        <template #header>
        ...
        <template #default="{ row }">
          <a v-if="row.low_chg > 0" :href="market" style="color: red">{{ row.low_chg }}</a>
          <a v-if="row.low_chg == 0" :href="market" style="color: gray">{{ row.low_chg }}</a>
          <a v-if="row.low_chg < 0" :href="market" style="color: green">{{ row.low_chg }}</a>
        </template>
      </el-table-column>
      <el-table-column width="70">
        <template #header>
        ...
        <template #default="{ row }">
          <a v-if="row.high_chg > 0" :href="market" style="color: red">{{ row.high_chg }}</a>
          <a v-if="row.high_chg == 0" :href="market" style="color: gray">{{ row.high_chg }}</a>
          <a v-if="row.high_chg < 0" :href="market" style="color: green">{{ row.high_chg }}</a>
        </template>
      </el-table-column>
      <el-table-column width="70">
        <template #header>
...
const axiosTable = ()=>{
  ...
  axios.post("http://127.0.0.1:8081/analysis/limit_market",query_dic).then(
      (response) => {
