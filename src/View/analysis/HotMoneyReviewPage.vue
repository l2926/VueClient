  ...
  <el-container>
    ...
    <el-table
      ...
      <el-table-column
          ...
          width="90">
        <template #default="{ row }">
          <el-link :href="`/#/market/daily?select_id=1&p\ara_id=20&level=analysis&id=${row.idx}&trade_date=${row.trade_date}`" style="color: gray" target="_blank">{{ row.ts_name }}</el-link>
        </template>
      </el-table-column>
...
<!--          <a :href="market" style="color: gray">{{ row.industry_name_l3 }}</a>-->
      ...
      <el-table-column width="60">
        ...
        <template #default="{ row }">
          <a :href="market" style="color: red">{{ row.buy_amount }}</a>
        </template>
      </el-table-column>
      <el-table-column width="60">
        <template #header>
        ...
        <template #default="{ row }">
          <a :href="market" style="color: green">{{ row.sell_amount }}</a>
        </template>
      </el-table-column>
      <el-table-column width="70">
        <template #header>
        ...
        <template #default="{ row }">
          <a v-if="row.net_amount > 0" :href="market" style="color: red">{{ row.net_amount }}</a>
          <a v-if="row.net_amount == 0" :href="market" style="color: gray">{{ row.net_amount }}</a>
          <a v-if="row.net_amount < 0" :href="market" style="color: green">{{ row.net_amount }}</a>
        </template>
      </el-table-column>
      <el-table-column
          prop="date"
